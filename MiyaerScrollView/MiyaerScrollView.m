...
#import "MiyaerCell.h"
@interface MiyaerScrollView ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
@end
@implementation MiyaerScrollView
...
- (void)setPageControlPositionType:(pageControlPositionType)pageControlPositionType{
...
-(void)initAttribute
{
    ...
    _infiniteLoop = NO;
    _pageControlPositionType = pageControlPositionTypeMiddle;    //pageControl的位置
    _currentIndex = 0;
}
#pragma mark - 创建子视图
...
-(void)createCycleView
{
    ...
    //注册轮播cell
    [_cycleCollectionView registerClass:[MiyaerCell class] forCellWithReuseIdentifier:@"MiyaerCell"];
}
...
-(void)createPageControl
{
    ...
    if (self.pageControlPositionType==pageControlPositionTypeLeft) {
            _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(20, CGRectGetMaxY(_cycleCollectionView.frame) - 30, 60, 30)];
    }else if (self.pageControlPositionType==pageControlPositionTypeMiddle){
            _pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width-60)/2, CGRectGetMaxY(_cycleCollectionView.frame) - 30, 60, 30)];
    }else{
    ...
    _pageControl.currentPageIndicatorTintColor = [UIColor yellowColor];//当前显示页的页码颜色
...
{
    MiyaerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"MiyaerCell" forIndexPath:indexPath];
    cell.placeNmae = _placeHolderImageName;
